...
//// LOLIN32
// Klok pin only needed for SPI based chipsets when not using hardware SPI
//#define CLK_PIN 12
...
void TheMatrix(uint16_t last_time_inc, uint16_t time_inc){
  ...
  while(!logical_equal(NUM_LEDS, wipe, have)){
	...
	if(n_drop > n_need){
	  ...
	  rows[i] = -random(0, MatrixHeight);
	  cols[i] = random(0, MatrixWidth); // random
	  //cols[i] = MatrixWidth - abs(i % (2*MatrixWidth) - MatrixWidth); // sawtooth
	}
      }
